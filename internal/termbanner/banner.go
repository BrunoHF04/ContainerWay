...
// ShellStart devolve o comando remoto: cd, MOTD com ASCII + stats ANSI, bash interativo.
func ShellStart(currentDir, sshHost, lang string) string {
	...
	b.WriteString(remoteColorWelcomeShell(LabelsFor(lang)))
	b.WriteString("\nexec bash -li")
...
func remoteColorWelcomeShell(lb Labels) string {
...
`,
		shellQuote(lb.Title),
		shellQuote(lb.System),
		shellQuote(lb.Now),
		shellQuote(lb.LineA),
		shellQuote(lb.LineBDisk),
		shellQuote(lb.LineBDiskPc),
		shellQuote(lb.LineBTemp),
		shellQuote(lb.LineBIP),
		shellQuote(lb.BootUptime),
		shellQuote(lb.Prompt),
	)
...
# Logo legível no topo (texto claro; evita figlet que distorce no browser)
print_cw_ascii() {
  printf "%b\n" "$c"
  printf '%s\n' \
    '+--------------------------------------------+' \
